...
public class ManagedConnection implements Connection {
    ...
    private Statement wrapStatement(Statement statement) {
        if (statement instanceof ManagedStatement) {
            return statement;
        }
        return new ManagedStatement(statement);
    }

    private PreparedStatement wrapPreparedStatement(PreparedStatement statement) {
        if (statement instanceof ManagedPreparedStatement) {
            return statement;
        }
        return new ManagedPreparedStatement(statement);
    }

    private CallableStatement wrapCallableStatement(CallableStatement statement) {
        if (statement instanceof ManagedCallableStatement) {
            return statement;
        }
        return new ManagedCallableStatement(statement);
    }
